...
// @version      1.0
// @description  Automatiserer aktiviteter og politi-håndtering i Mafiaens Hevn nettleserspill
// @author       Jarl
...
(function () {
    ...
    function waitForPageLoad(callback) {
        const checkReady = setInterval(() => {
            const neededEl = document.querySelector("body");
            if (neededEl) {
    ...
    waitForPageLoad(() => {
        ...
        function createControlPanel() {
            ...
            // Finn menyboksen basert på unik tekst og klasse
            const menuBox = Array.from(document.querySelectorAll("div"))
                .find(el =>
                    el.textContent.includes("Penger") &&
                    el.textContent.includes("Politi") &&
                    getComputedStyle(el).backgroundColor === "rgba(17, 17, 17, 0.9)"
                );

            if (menuBox) {
                // Sett stilen relativt til menyboks
                const rect = menuBox.getBoundingClientRect();
                panel.style.position = "absolute";
                panel.style.top = `${rect.bottom + window.scrollY + 8}px`;
                panel.style.left = `${rect.left + window.scrollX}px`;
            } else {
                // Fallback hvis meny ikke finnes
                panel.style.position = "fixed";
                panel.style.top = "11px";
                panel.style.right = "11px";
            }

            // Stil som matcher spillet
            panel.style.background = "rgba(17, 17, 17, 0.95)";
            panel.style.border = "1px solid #04354a";
            ...
            panel.style.boxShadow = "0 0 4px #000";

            panel.innerHTML = `
    <div style="margin-bottom: 6px; font-weight: bold; border-bottom: 1px solid #04354a; padding-bottom: 4px;">
      Mafiaens Hevn Auto
    </div>
    <div id="mh-status">Status: <span style="color:lime">Starter...</span></div>
    ...
    <div style="margin-top:8px; display: flex; gap: 4px; flex-wrap: wrap;">
      <button id="mh-stop">⏹ Stopp</button>
  ...
  `;
            ...
            // ... resten av knappelogikken (kan stå som før)
        ...
        if (!localStorage.getItem("mh_fallback")) {
            localStorage.setItem("mh_fallback", JSON.stringify([]));
        }
        ...
        async function scanAllAvailableActivities() {
            ...
            allActivities = uniqueResults.map(({ menu, name }) => ({ menu, name }));
            localStorage.setItem("mh_allActivities", JSON.stringify(allActivities));


            // Bare hvis du ønsker å fylle fallback med alt funnet:
            if (fallbackPriority.length === 0) {
                fallbackPriority = allActivities.map(a => `${a.menu} → ${a.name}`);
                localStorage.setItem("mh_fallback", JSON.stringify(fallbackPriority));
            }
